...
namespace SuperTiled2Unity {
    public class SuperMap : MonoBehaviour {
        [ReadOnly] public string m_Version;

        [ReadOnly] public string m_TiledVersion;

        [ReadOnly] public MapOrientation m_Orientation;

        [ReadOnly] public MapRenderOrder m_RenderOrder;

        [ReadOnly] public int m_Width;

        [ReadOnly] public int m_Height;

        [ReadOnly] public int m_TileWidth;

        [ReadOnly] public int m_TileHeight;

        [ReadOnly] public int m_HexSideLength;

        [ReadOnly] public StaggerAxis m_StaggerAxis;

        [ReadOnly] public StaggerIndex m_StaggerIndex;

        [ReadOnly] public bool m_Infinite;

        [ReadOnly] public Color m_BackgroundColor;

        [ReadOnly] public int m_NextObjectId;
        ...
        private void Start() {
            ...
            foreach (var renderer in GetComponentsInChildren<TilemapRenderer>()) renderer.detectChunkCullingBounds = TilemapRenderer.DetectChunkCullingBounds.Auto;
        }
        ...
        private Vector3Int TiledCellToGridCell(int x, int y) {
            if (m_Orientation == MapOrientation.Isometric) return new Vector3Int(-y, x, 0);
