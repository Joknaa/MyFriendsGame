...
namespace SuperTiled2Unity.Editor {
    ...
    public class TilesetLoader {
        ...
        private void ProcessAttributes(XElement xTileset) {
            ...
            m_TilesetScript.m_ObjectAlignment = xTileset.GetAttributeAs("objectalignment", ObjectAlignment.Unspecified);

            var xTileOffset = xTileset.Element("tileoffset");
            if (xTileOffset != null) {
                var x = xTileOffset.GetAttributeAs("x", 0.0f);
                var y = xTileOffset.GetAttributeAs("y", 0.0f);
            ...
            if (xGrid != null) {
                ...
                var w = xGrid.GetAttributeAs("width", 0.0f);
                var h = xGrid.GetAttributeAs("height", 0.0f);
        ...
        private void BuildTileset(XElement xTileset) {
            ...
            var atlas = new AtlasBuilder(m_Importer, m_UseSpriteAtlas, m_AtlasWidth, m_AtlasHeight, m_TilesetScript);
        ...
        private void BuildTilesetFromImage(XElement xTileset, AtlasBuilder atlas) {
            ...
            var textureAssetPath = xImage.GetAttributeAs<string>("source");
            var textureWidth = xImage.GetAttributeAs<int>("width");
            var textureHeight = xImage.GetAttributeAs<int>("height");
            ...
            if (imgHeaderDims.x != textureWidth || imgHeaderDims.y != textureHeight) {
                ...
                m_Importer.ReportError(
                    "Mismatching width/height detected. Tileset = ({0}, {1}), image = {2}. This may happen when a tileset image has been resized. Open map and tileset in Tiled Map Editor and resave.",
                    textureWidth, textureHeight, imgHeaderDims);
                m_TilesetScript.m_HasErrors = true;
                return;
            }
            ...
            for (var i = 0; i < m_TilesetScript.m_TileCount; i++) {
                // Get grid x,y coords
                var x = i % m_TilesetScript.m_TileColumns;
                var y = i / m_TilesetScript.m_TileColumns;

                // Get x source on texture
                var srcx = x * m_TilesetScript.m_TileWidth;
                srcx += x * m_TilesetScript.m_Spacing;
                srcx += m_TilesetScript.m_Margin;

                // Get y source on texture
                var srcy = y * m_TilesetScript.m_TileHeight;
                ...
                var rcSource = new Rect(srcx, srcy, m_TilesetScript.m_TileWidth, m_TilesetScript.m_TileHeight);
        ...
        private void BuildTilesetFromCollection(XElement xTileset, AtlasBuilder atlas) {
            ...
            foreach (var xTile in xTileset.Elements("tile")) {
                var tileIndex = xTile.GetAttributeAs<int>("id");
        ...
        private void ProcessTileElements(XElement xTileset) {
            ...
            foreach (var xTile in xTileset.Elements("tile")) ProcessTileElement(xTile);
        }

        private void ProcessTileElement(XElement xTile) {
            var index = xTile.GetAttributeAs("id", -1);
            ...
            if (m_TilesetScript.TryGetTile(index, out tile)) {
                ...
                if (xAnimation != null) ProcessAnimationElement(tile, xAnimation);

                // Does the tile have an object group?
                var xObjectGroup = xTile.Element("objectgroup");
                if (xObjectGroup != null && xObjectGroup.HasElements) ProcessObjectGroupElement(tile, xObjectGroup);
            }
        }
        ...
        private void ProcessAnimationElement(SuperTile tile, XElement xAnimation) {
            ...
            foreach (var xFrame in xAnimation.Elements("frame")) {
                ...
                SuperTile frame;
                if (m_TilesetScript.TryGetTile(frameId, out frame)) animations.AddFrames(frame.m_Sprite, frameDuration);
            }
            ...
            if (xObjectGroup.Elements().Any()) {
                ...
                foreach (var xObject in xObjectGroup.Elements("object")) {
                    ...
                    if (xPolygon != null) {
                        ...
                        if (PolygonUtils.SumOverEdges(points) < 0) points = points.Reverse().ToArray();
        ...
        private void AssignCollisionObjectProperties(CollisionObject collision, SuperTile tile) {
            ...
            if (layerProperty != null) {
                ...
                if (LayerMask.NameToLayer(collision.m_ObjectName) != -1) collision.m_PhysicsLayer = collision.m_ObjectName;
            }

            if (string.IsNullOrEmpty(collision.m_PhysicsLayer)) collision.m_PhysicsLayer = "Default";
            ...
            if (triggerProperty != null) collision.m_IsTrigger = triggerProperty.GetValueAsBool();
        }
        ...
        private CustomProperty GetCollisionOrTileOrTilesetProperty(CollisionObject collision, SuperTile tile, string propertyName) {
            ...
            if (collision.m_CustomProperties.TryGetProperty(propertyName, out property)) return property;

            // Get property off the tile
            if (tile.m_CustomProperties.TryGetProperty(propertyName, out property)) return property;

            // Inherit property from the tileset
            if (m_TilesetScript.m_CustomProperties.TryGetProperty(propertyName, out property)) return property;
