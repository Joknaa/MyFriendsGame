...
namespace SuperTiled2Unity.Editor {
    public static class ColliderGizmos {
        ...
        public static void DrawColliderShapes(SuperColliderComponent collider, ST2USettings settings) {
            foreach (var shape in collider.m_PolygonShapes) FillShape(collider.gameObject, shape.m_Points, settings);

            foreach (var shape in collider.m_OutlineShapes) OutlineShape(collider.gameObject, shape.m_Points, settings);
        }

        public static void DrawColliders(GameObject go, ST2USettings settings) {
            foreach (var polygon in go.GetComponentsInChildren<PolygonCollider2D>()) DrawPolygon(polygon, settings);

            foreach (var box in go.GetComponentsInChildren<BoxCollider2D>()) DrawBox(box, settings);

            foreach (var circle in go.GetComponentsInChildren<CircleCollider2D>()) DrawCircle(circle, settings);

            foreach (var edge in go.GetComponentsInChildren<EdgeCollider2D>()) DrawLines(edge, settings);
        }
        ...
        private static void DrawPolygon(PolygonCollider2D polygon, ST2USettings settings) {
            ...
            var offset = polygon.transform.TransformVector(polygon.offset);
        ...
        private static void DrawLines(EdgeCollider2D edge, ST2USettings settings) {
            var offset = edge.transform.TransformVector(edge.offset);
        ...
        private static Color GetColorFromObject(GameObject go, ST2USettings settings) {
            ...
            if (objectLayer != null) {
                ...
                if (superObject != null) {
                    ...
                    if (settings.CustomObjectTypes.TryGetCustomObjectType(superObject.m_Type, out objectType)) color = objectType.m_Color;
                }
