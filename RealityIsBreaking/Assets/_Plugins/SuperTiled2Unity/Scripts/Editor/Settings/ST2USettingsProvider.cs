...
namespace SuperTiled2Unity.Editor {
    public class ST2USettingsProvider : SettingsProvider {
        ...
        public override void OnGUI(string searchContext) {
            ...
            if (m_S2TUSettingsObject.ApplyModifiedProperties()) ST2USettings.GetOrCreateST2USettings().RefreshCustomObjectTypes();
        }

        public override void OnTitleBarGUI() {
            var tex = EditorGUIUtility.FindTexture("_Help");
            var content = new GUIContent(tex, "Go to SuperTiled2Unity Documentation");
            if (GUILayout.Button(content, EditorStyles.helpBox)) Application.OpenURL("https://supertiled2unity.readthedocs.io");
        }
        ...
        private void DoGuiSettings() {
            ...
            if (EditorGUI.EndChangeCheck()) ppuProperty.floatValue = Mathf.Clamp(ppuProperty.floatValue, 0.01f, 2048);

            // Edges Per Ellipse
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(edgesProperty, SettingsContent.m_EdgesPerEllipseContent);
            if (EditorGUI.EndChangeCheck()) edgesProperty.intValue = Mathf.Clamp(edgesProperty.intValue, 6, 256);
            ...
            if (EditorGUI.EndChangeCheck()) animationPrpoerty.intValue = Mathf.Clamp(animationPrpoerty.intValue, 1, 125);

            EditorGUILayout.HelpBox("In frames-per-second. Note: You will need to reimport all your tilesets after making changes to the animation framerate for tiles.",
                MessageType.None);
        }
        ...
        private void DoGuiMaterialMatchings() {
            ...
            if (m_ShowMaterialMatchings) {
                ...
                EditorGUILayout.HelpBox(
                    "Fill this out with the names of Tile Layer names and their matching materials. Any imported Tiled Layer with a matching name will use the assigned material.",
                    MessageType.None);
            }
        }
        ...
        private void DoGuiPrefabReplacements() {
            ...
            if (m_ShowPrefabReplacements) {
                ...
                EditorGUILayout.HelpBox(
                    "When the Tiled import scripts come across a Tiled Object of one of these given types it will be replaced, automatically, with the associated prefab.",
                    MessageType.None);
            }
        }
        ...
        private void DoGuiColliders() {
            ...
            var geoTypeProperty = m_S2TUSettingsObject.FindProperty("m_CollisionGeometryType");
            geoTypeProperty.intValue = (int) (CompositeCollider2D.GeometryType) EditorGUILayout.EnumPopup(SettingsContent.m_CollisionGeometryTypeContent,
                (CompositeCollider2D.GeometryType) geoTypeProperty.intValue);

            m_ShowLayerColors = EditorGUILayout.Foldout(m_ShowLayerColors, SettingsContent.m_LayerColorsContent);
            if (m_ShowLayerColors) {
                var listProperty = m_S2TUSettingsObject.FindProperty("m_LayerColors");
                ...
                using (new GuiScopedIndent()) {
                    for (var i = 0; i < 32; i++) {
                        ...
                        var indexProperty = listProperty.GetArrayElementAtIndex(i);
                        var indexPropertyContext = new GUIContent(layerName, string.Format("Select color for {0} tile layer colliders", layerName));
        ...
        private void DoCustomPropertySettings() {
            ...
            if (!string.IsNullOrEmpty(m_ST2USettings.ParseXmlError)) EditorGUILayout.HelpBox(m_ST2USettings.ParseXmlError, MessageType.Error);

            EditorGUILayout.Space();
            using (new GUILayout.HorizontalScope()) {
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("Refresh")) ST2USettings.GetOrCreateST2USettings().RefreshCustomObjectTypes();

                if (GUILayout.Button("View Custom Properties")) CustomPropertiesWindow.ShowCustomPropertiesWindow();
            }
        }
        ...
        private void DoGuiReimportAssets() {
            ...
            using (new GUILayout.HorizontalScope()) {
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("Reimport Tiled Assets")) ReimportTiledAssets(m_ApplyDefaultSettings);
            }
        }
        ...
        private void OnDrawMaterialMatchingElement(Rect rect, int index, bool isActive, bool isFocused) {
            using (new GuiScopedIndent()) {
                ...
                var fieldWidth = (rect.width - kMargin) / 2;
        ...
        private static void ReimportTiledAssets(bool applyDefaults) {
            if (applyDefaults)
                foreach (var guid in AssetDatabase.FindAssets("t:SuperAsset")) {
                    ...
                    if (importer != null) importer.ApplyDefaultSettings();
                }
        ...
        public class SettingsContent {
            public static readonly GUIContent m_PixelsPerUnitContent =
                new GUIContent("Default Pixels Per Unit", "How many pixels in the sprite correspond to one unit in the world. (Default Setting)");

            public static readonly GUIContent m_EdgesPerEllipseContent =
                new GUIContent("Default Edges Per Ellipse", "How many edges to use when appromixating ellipse/circle colliders. (Default Setting)");
            ...
            public static readonly GUIContent m_DefaultMaterialContent = new GUIContent("Default Material",
                "Set to the material you want to use for sprites and tiles imported by SuperTiled2Unity. Leave empy to use built-in sprite material.");
            ...
            public static readonly GUIContent m_ObjectTypesXmlContent =
                new GUIContent("Object Types Xml", "Set to an Object Types Xml file exported from Tiled Object Type Editor.");
            ...
            public static readonly GUIContent m_CollisionGeometryTypeContent =
                new GUIContent("Collision Geometry Type", "The type of geometry used by CompositeCollider2D components.");
            ...
            public static readonly GUIContent m_ApplyDefaultSettingsContent =
                new GUIContent("Apply Default Settings", "Default Import Settings will be applied to every ST2U that is imported. Beware!");
        }
    }
}
#endif