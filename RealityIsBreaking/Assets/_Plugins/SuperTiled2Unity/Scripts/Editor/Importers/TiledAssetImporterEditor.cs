...
namespace SuperTiled2Unity.Editor {
    public abstract class TiledAssetImporterEditor<T> : SuperImporterEditor<T> where T : SuperImporter {
        ...
        protected void ShowTiledAssetGui() {
            ...
            if (EditorGUI.EndChangeCheck()) m_PixelsPerUnit.floatValue = Mathf.Clamp(m_PixelsPerUnit.floatValue, 0.01f, 2048);

            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(m_EdgesPerEllipse, m_EdgesPerEllipseContent);
            if (EditorGUI.EndChangeCheck()) m_EdgesPerEllipse.intValue = Mathf.Clamp(m_EdgesPerEllipse.intValue, 6, 256);
        }
