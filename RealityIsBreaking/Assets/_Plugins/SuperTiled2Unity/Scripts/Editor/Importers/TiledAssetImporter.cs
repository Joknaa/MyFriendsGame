...
namespace SuperTiled2Unity.Editor {
    public abstract class TiledAssetImporter : SuperImporter {
        [SerializeField] private float m_PixelsPerUnit;

        [SerializeField] private int m_EdgesPerEllipse;

#pragma warning disable 414
        [SerializeField] private int m_NumberOfObjectsImported;
#pragma warning restore 414
        ...
        public void AddSuperCustomProperties(GameObject go, XElement xProperties, SuperTile tile, string typeName) {
            ...
            if (tile != null) properties.CombineFromSource(tile.m_CustomProperties);
        ...
        public void AssignMaterial(Renderer renderer, string match) {
            ...
            if (SuperImportContext.Settings.DefaultMaterial != null) renderer.material = SuperImportContext.Settings.DefaultMaterial;
        }
        ...
        protected void AssignUnityLayer(SuperCustomProperties properties) {
            ...
            if (properties.TryGetCustomProperty(StringConstants.Unity_Layer, out prop)) {
                var layer = prop.m_Value;
                if (!InternalEditorUtility.layers.Contains(layer)) {
                    var report = string.Format("Layer '{0}' is not defined in the Tags and Layers settings.", layer);
                    ReportError(report);
                }
                else {
                ...
                if (parent != null) properties.gameObject.layer = parent.gameObject.layer;
            }
        }
        ...
        private void WrapImportContext(AssetImportContext ctx) {
            ...
            if (m_PixelsPerUnit == 0) m_PixelsPerUnit = settings.PixelsPerUnit;

            if (m_EdgesPerEllipse == 0) m_EdgesPerEllipse = settings.EdgesPerEllipse;
