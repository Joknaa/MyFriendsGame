...
namespace SuperTiled2Unity.Editor {
    ...
    internal class TmxAssetImporterEditor : TiledAssetImporterEditor<TmxAssetImporter> {
        ...
        private readonly GUIContent m_TilesAsObjectsContent = new GUIContent("Tiles as Objects",
            "Place each tile as separate game object. Uses more resources but gives you more control. This is ignored for Isometric maps that are forced to use game objects.");
        ...
        private void EnumerateCustomImporterClasses() {
            ...
            var customTypes = AppDomain.CurrentDomain.GetAllDerivedTypes<CustomTmxImporter>().Where(t => t.IsClass && !t.IsAbstract && t.IsSubclassOf(typeof(CustomTmxImporter)))
                .Where(t => t.GetCustomAttributes(typeof(AutoCustomTmxImporterAttribute), true).Length == 0).OrderBy(t => t.GetDisplayName());
        ...
        private void ShowCustomImporterGui() {
            ...
            EditorGUILayout.HelpBox(
                "Custom Importers are an advanced feature that require scripting. Create a class inherited from CustomTmxImporter and select it from the list above.",
                MessageType.None);
            ...
            using (new GuiScopedIndent()) {
                ...
                if (m_ShowAutoImporters) {
                    foreach (var t in importers) EditorGUILayout.LabelField(t.GetDisplayName());

                    EditorGUILayout.HelpBox(
                        "Auto Importers are custom importers that run on automatically on every exported Tiled map. Order is controlled by the AutoCustomTmxImporterAttribute.",
                        MessageType.None);
                }
            }
        }
