...
namespace SuperTiled2Unity {
    public class SuperTile : TileBase {
        ...
        [ReadOnly] public int m_TileId;

        [ReadOnly] public Sprite m_Sprite;

        [ReadOnly] public Sprite[] m_AnimationSprites;

        [ReadOnly] public string m_Type;

        [ReadOnly] public float m_Width;

        [ReadOnly] public float m_Height;

        [ReadOnly] public float m_TileOffsetX;

        [ReadOnly] public float m_TileOffsetY;

        [ReadOnly] public ObjectAlignment m_ObjectAlignment;
        ...
        public Matrix4x4 GetTransformMatrix(FlipFlags ff, MapOrientation orientation) {
            ...
            if (ff == FlipFlags.None) return matOffset;

            var flipHorizontal = FlipFlagsMask.FlippedHorizontally(ff);
            var flipVertical = FlipFlagsMask.FlippedVertically(ff);
            var flipDiagonal = FlipFlagsMask.RotatedDiagonally(ff);
            var rotateHex120 = FlipFlagsMask.RotatedHexagonally120(ff);

            var width = m_Width * inversePPU;
            var height = m_Height * inversePPU;

            var tileCenter = new Vector2(width, height) * 0.5f;

            var matTransIn = Matrix4x4.identity;
            var matFlip = Matrix4x4.identity;
            var matTransOut = Matrix4x4.identity;

            // Go to the tile center
            matTransIn = Matrix4x4.Translate(-tileCenter);

            if (orientation == MapOrientation.Hexagonal) {
                if (flipDiagonal) matFlip *= Rotate60Matrix;

                if (rotateHex120) matFlip *= Rotate120Matrix;

                if (flipHorizontal) matFlip *= HorizontalFlipMatrix;

                if (flipVertical) matFlip *= VerticalFlipMatrix;
                ...
                if (flipDiagonal) matFlip *= DiagonalFlipMatrix;
        ...
        public void GetTRS(FlipFlags flags, MapOrientation orientation, out Vector3 xfTranslate, out Vector3 xfRotate, out Vector3 xfScale) {
            ...
            var flippedHorizontally = FlipFlagsMask.FlippedHorizontally(flags);
            var flippedVertically = FlipFlagsMask.FlippedVertically(flags);
            var rotatedDiagonally = FlipFlagsMask.RotatedDiagonally(flags);
            var rotatedHexagonally120 = FlipFlagsMask.RotatedHexagonally120(flags);
            ...
            if (flags != FlipFlags.None) {
                if (orientation == MapOrientation.Hexagonal) {
                    if (rotatedDiagonally) xfRotate.z -= 60;

                    if (rotatedHexagonally120) xfRotate.z -= 120;
                }
